...
st.markdown("Upload or paste your Loan Estimate (LE) or Closing Disclosure (CD) text below for an instant plain-language breakdown and TRID tolerance check.")

api_key = st.secrets.get("ANTHROPIC_API_KEY") or st.text_input("Enter Anthropic API Key:", type="password")
raw_doc_text = st.text_area("Paste Raw Document Text Here:", height=200)

if st.button("Analyze Document", type="primary"):
if not api_key:
    st.error("Please provide an Anthropic API key to proceed.")
elif not raw_doc_text:
    st.warning("Please enter some document text to analyze.")
else:
